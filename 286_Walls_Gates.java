 ...
 * SOLUTION AND LOGIC:
 * 
 * TRAVERSE THE ROOMS MATRIX ASND AS SOON ASNYOU FIND A GATE IF ANY NUMBER WITH VALUE 0 THEN 
 * APPLY DFS AND ALSO CALCUKATE THE DISTANCE MIN FROM ALL THE 4 DIRECTIONS AND RETURN AND UPDATE THAT ARRAY
 */

class walls_gates_dfs {
    ...
    public void wallsAndGates(int[][] rooms) {
        ...
        for(int i = 0; i < rooms.length; i++) {
            ...
            for(int j = 0; j < rooms[0].length; j++) {
                ...
                //CHECK IF THE VALUE IS A GATE ?? IE ITS 0, IF YES THEN APPLY DFS
                if(rooms[i][j] == 0) {
                    dfs(rooms, i, j, 0);
                }
            }
    ...
    void dfs(int[][] rooms, int i, int j, int distance) {
        ...
        if(i < 0 || i >= rooms.length || j < 0 || j >= rooms[0].length || rooms[i][j] < distance) {
            //BOUNDRY CASE IF THE TRAVESRSE IS OUT OF THE MATRIX
            //OR ITS THE WORST DISTANCE THAT IS ROOM[I][J] < DISTANCE
            return;
        }
